...
/**
 * Class that handles the /distance endpoint that retrieves the distances between the source location
 * and other inputted location. Contains an instance of DistanceDataSource to obtain distance values
 */
public class DistanceHandler implements Route {

  /**
   * Handle method that converts the inputted locations into a list of strings and uses them to
   * find coordinates and distances. If distances are found successfully, a success message will
   * be returned with the distances (list of doubles). Otherwise, a failure response will returned
  ...
  /**
   * A record representing a successful call to the /distance handler, containing a result of success,
