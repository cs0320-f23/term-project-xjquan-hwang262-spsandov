...
test("Test csv data table is being produced", async ({ page }) => {
  ...
  await expect(page.getByRole('cell', { name: 'Column1' }).nth(1)).toBeVisible();
});
...
test("test invalid search command", async ({ page }) => {
  ...
  await page.getByLabel("Command input").fill("mock_load with_header/header_multiple_line.csv");
  await page.locator("button").click();
  await page.waitForTimeout(100);

  await page.waitForSelector(".repl-history");

  await page.getByLabel("Command input").fill("mock_search nonexistent nonexistent");
  await page.locator("button").click();
  await page.waitForSelector(".repl-history");
  await expect(page.locator(".repl-history")).toContainText("Error: No results found for the specified search criteria");

  await page.getByLabel("Command input").fill("mock_search 1 Value1 test");
  await page.locator("button").click();
  await page.waitForSelector(".repl-history");
  await expect(page.locator(".repl-history")).toContainText("Error: Invalid search command format");
});
